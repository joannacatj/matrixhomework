...
class TrainerDifIR(TrainerBase):
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            text_names = [str(int(value.item())) + '.txt' for value in micro_data['text']]
            ...
            for i in range(len(text_file_paths)):
                ...
                with open(text_file_path, 'r') as f:
                    ...
                    text_batch.append(selected_text)
            # print(text_batch)
            ...
            if self.configs.train.use_fp16:
                ...
                gt_list = []
                for i in range(micro_data['gt'].size(0)):
                    gt_tensor = micro_data['gt'][i]
                    gt_list_pil = to_pil_image(gt_tensor)
                    gt_list.append(gt_list_pil)

                clip_loss_text=calculate_clip_loss(image_list,text_batch)


                loss = (losses["loss"] + clip_loss_text).mean() / num_grad_accumulate
                print(loss)
                loss.backward()
